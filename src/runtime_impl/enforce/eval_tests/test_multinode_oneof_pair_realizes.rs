...
/// when both dir and file are present, pair realizes and single does not (inner one_of fires 2).
/// realized=1 → no CardinalityViolation. Both dir and file are declared → no Undeclared.
#[test]
fn test_multinode_oneof_pair_realizes() {
    // Arrange
    let tree = DirTree {
        ...
        files: vec!["x.txt".to_string()],
    };

    // pair alternative: subtree declares both dir:x and file:x.txt
    ...
    assert!(
        errors.is_empty(),
        "expected no errors when dir+file both present (pair realizes, single does not): {:?}",
        errors
    );
}
