...
/// one_of (min=1, max=1): both alternatives exist → CardinalityViolation
#[test]
fn test_oneof_exactly_one_both_match_violation() {
    // Arrange
    let tree = DirTree {
        name: "root".to_string(),
        dirs: vec![],
        files: vec!["config.toml".to_string(), "config.yaml".to_string()],
    };
    ...
    assert_eq!(
        violations.len(),
        1,
        "expected CardinalityViolation when both alternatives exist: {:?}",
        errors
    );
