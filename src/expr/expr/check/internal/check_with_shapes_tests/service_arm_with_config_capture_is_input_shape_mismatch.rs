...
/// Inside the `service` arm, `${c.regex.cfg}` references the capture of the `config` alternative
/// — must produce `WithShapeMismatch` (E018).
#[test]
fn service_arm_with_config_capture_is_input_shape_mismatch() {
    // Arrange: service arm incorrectly uses ${c.regex.cfg} (cfg is a config-alt capture, not service)
    let service_arm = arm_entry("service", vec![file_leaf("${c.regex.cfg}.md")]);
    ...
    // Assert: exactly one WithShapeMismatch (in the service arm)
