...
/// Adding count to a splice (bare rule) entry is InvalidPattern.
#[test]
fn adding_count_to_splice_is_error() {
    // Arrange: bare rule splice entry with count
    let entry = YamlEntry {
        id: None,
        optional: None,
        min: None,
    ...
    assert!(
        ...
        "expected InvalidPattern for adding count to splice: {:?}",
        errors
    );
}
