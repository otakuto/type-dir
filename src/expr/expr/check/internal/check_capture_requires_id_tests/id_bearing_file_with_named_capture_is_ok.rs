...
/// An id-bearing file entry with a named capture is valid (no error).
#[test]
fn id_bearing_file_with_named_capture_is_ok() {
    // Arrange
    let entry = YamlEntry {
        id: Some(EntryId("myfile".to_string())),
        optional: None,
        min: None,
        max: None,
        count: None,
        kind: YamlEntryKind::File {
            pattern: YamlPattern::Spec(PatternSpec {
                regex: Some(RegexPattern(r"^(?<stem>[a-z]+)\.rs$".to_string())),
            }),
            body: None,
